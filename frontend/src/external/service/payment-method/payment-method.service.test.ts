...
describe('listPaymentMethods', () => {
  it('一覧を正常に取得できる', async () => {
    vi.mocked(paymentMethodsListPaymentMethods).mockResolvedValue({ status: 200, data: [mockPaymentMethod] } as any)
  ...
  it('200以外のステータスでエラーをスローする', async () => {
    vi.mocked(paymentMethodsListPaymentMethods).mockResolvedValue({ status: 500, data: null } as any)
...
describe('getPaymentMethodById', () => {
  it('支払い方法を正常に取得できる', async () => {
    vi.mocked(paymentMethodsGetPaymentMethod).mockResolvedValue({ status: 200, data: mockPaymentMethod } as any)
  ...
  it('200以外のステータスでnullを返す', async () => {
    vi.mocked(paymentMethodsGetPaymentMethod).mockResolvedValue({ status: 404, data: null } as any)
...
describe('createPaymentMethod', () => {
  it('支払い方法を正常に作成できる', async () => {
    vi.mocked(paymentMethodsCreatePaymentMethod).mockResolvedValue({ status: 201, data: mockPaymentMethod } as any)
  ...
  it('201以外のステータスでエラーをスローする', async () => {
    vi.mocked(paymentMethodsCreatePaymentMethod).mockResolvedValue({ status: 400, data: null } as any)

    await expect(createPaymentMethod({ name: 'カード' })).rejects.toThrow('支払い方法の作成に失敗しました')
  })
})
...
describe('updatePaymentMethod', () => {
  it('支払い方法を正常に更新できる', async () => {
    ...
    vi.mocked(paymentMethodsUpdatePaymentMethod).mockResolvedValue({ status: 200, data: updated } as any)

    const result = await updatePaymentMethod('123e4567-e89b-12d3-a456-426614174000', { name: '新しいカード' })
    expect(result.name).toBe('新しいカード')
  })

  it('200以外のステータスでエラーをスローする', async () => {
    vi.mocked(paymentMethodsUpdatePaymentMethod).mockResolvedValue({ status: 404, data: null } as any)

    await expect(updatePaymentMethod('id', { name: 'カード' })).rejects.toThrow('支払い方法の更新に失敗しました')
  })
})

describe('deletePaymentMethod', () => {
  it('正常に削除できる', async () => {
    vi.mocked(paymentMethodsDeletePaymentMethod).mockResolvedValue({ status: 204, data: null } as any)

    await expect(deletePaymentMethod('123e4567-e89b-12d3-a456-426614174000')).resolves.toBeUndefined()
  })

  it('204以外のステータスでエラーをスローする', async () => {
    vi.mocked(paymentMethodsDeletePaymentMethod).mockResolvedValue({ status: 404, data: null } as any)
...
describe('deletePaymentMethods', () => {
  ...
  it('複数IDを正常に削除できる', async () => {
    vi.mocked(paymentMethodsDeletePaymentMethods).mockResolvedValue({ status: 204, data: null } as any)
  ...
  it('204以外のステータスでエラーをスローする', async () => {
    vi.mocked(paymentMethodsDeletePaymentMethods).mockResolvedValue({ status: 500, data: null } as any)
